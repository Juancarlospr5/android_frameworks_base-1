...
public class CompassTile extends QSTileImpl<BooleanState> implements SensorEventListener {
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        if (state.value) {
            ...
            if (arg != null) {
                state.label = formatValueWithCompassDirection(degrees);
    ...
    private String formatValueWithCompassDirection(float degree) {
        int compassDirectionIndex = (int) (Math.floor(((degree - 22.5) % 360) / 45) + 1) % 8;
        String[] compassDirections = mContext.getResources().getStringArray(
                R.array.compass_directions);

        return mContext.getString(R.string.quick_settings_compass_value, degree,
                compassDirections[compassDirectionIndex]);
    }
